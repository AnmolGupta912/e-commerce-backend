...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (existedUser) {
    throw new ApiError(
      409,
      "User with the same email already exist in Database!!!"
    );
  }
...
const changePassword = asyncHandler(async (req, res) => {
  ...
  // get the old password and new password from req.body
  // check if the old password is correct
  // if not throw error
  // if yes update the password with new password
  ...
  const isPasswordValid = await User.findById(userId).then((user) => {
...
const deleteUser = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndDelete(userId, { new: true }).select("-password -refreshToken -wishList");
